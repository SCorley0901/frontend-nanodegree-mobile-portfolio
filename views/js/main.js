...
var pizzaElementGenerator = function(i) {
  ...
  // Pizza id was not used so I commented this out
  //pizzaContainer.id = "pizza" + i;                // gives each pizza element a unique id
  pizzaImageContainer.classList.add("col-md-6");

  // use a compressed image
...
// I reworked the resizePizzes function quite a bit. I took out unneeded function definitions and calls
// this improved speed by almost .5ms
    switchSizeTo = 0
    switch(size) {
      case "1":
...
//  I also removed variables and put calculations together
    var windowWidth = document.querySelector("#randomPizzas").offsetWidth;

  // Iterates through pizza elements on the page and changes their widths
    //I changed newwidth to just be based on the first pizza element size instead of looking at each of them
    //setup a new variable to hold the pizza elements - this prevents thrashing
    var pizzaElements = document.querySelectorAll(".randomPizzaContainer");
    var pizzaElement = pizzaElements[0];
    var newWidth = (pizzaElement.offsetWidth + ((switchSizeTo - pizzaElement.offsetWidth/windowWidth)*windowWidth)) + 'px';
...
// Moved the read dom action out of the loop - prevents thrashing
// Set frag as a document fragment to minimize reflow
// Loop control variable added to slightly improve loop speed - numberOfPIzzasToGenerate
var numberOfPIzzasToGenerate = 100
var pizzasDiv = document.getElementById("randomPizzas");
var frag = document.createDocumentFragment();
// Append the pizzas generated to the fragment
for (var i = 2; i < numberOfPIzzasToGenerate; i++) {
...
// Reset animation call variable newFrame. This allows the animation to continue after this frame is written to the DOM
  newFrame = false;
...
// Moved this calculation out of the loop
  ...
  for (var i = 0; i < numberOfMovingPIzzas; i++) {
    var left = (100 * Math.sin(latestKnownScrollY + (i % 5)));
    // Changed from style.left to style.transform
    items[i].style.WebkitTransform = 'translateX('+ left + 'px)';
  }
...
// This next two variables and the following two functions use requestAnimationFrame to decouple the
//  sliding pizzas from the scroll event. This prevents stacking scroll events. I found this website very helpful
...
function onScroll(timestamp) {
  ...
  // this if statement checks to see if a scroll event is already being processed. If not then process the animation changes
  if(!newFrame) {
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var cols = 8;
  var s = 256;
// Loop control variable added to slightly improve loop speed - numberOfMovingPIzzas
  numberOfMovingPIzzas = 30
//moverfrag is a document fragment used to minimize writes to the document
  var moverFrag = document.createDocumentFragment();
//Changed the loop from 200 to 30.
  for (var i = 0; i < numberOfMovingPIzzas; i++) {
    var elem = document.createElement('img');
    elem.className = 'mover';
// Resized the image to 100 by 73 and moved it to mover css background
    elem.src = "images/compressed/pizza_small.png";
// Moved to CSS
...
//    elem.style.width = "73.333px";
    ...
    moverFrag.appendChild(elem);
  }
  document.querySelector("#movingPizzas1").appendChild(moverFrag);
  updatePositions();
});
